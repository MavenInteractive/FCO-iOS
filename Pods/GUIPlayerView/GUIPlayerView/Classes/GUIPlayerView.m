...
@interface GUIPlayerView () <AVAssetResourceLoaderDelegate, NSURLConnectionDataDelegate, AVAudioPlayerDelegate>
...
- (void)addWatermark {
    ...
    double delayInSeconds = 0.0f; //0.1;
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds *   NSEC_PER_SEC));
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
       [self play];
        
        // add watermark when playing
        UIImage *myImage = [UIImage imageNamed:@"detail_img_fightcallout"];
        CALayer *aLayer = [CALayer layer];
        aLayer.contents = (id)myImage.CGImage;
        aLayer.frame = CGRectMake(10, 46, 57, 57); //Needed for proper display. We are using the app icon (57x57). If you use 0,0 you will not see it
        aLayer.opacity = 0.65; //Feel free to alter the alpha here
        [playerLayer addSublayer:aLayer];
    });
}
    ...
    [asset loadValuesAsynchronouslyForKeys:keys completionHandler:^{
        ...
        if (playAutomatically) {
            dispatch_sync(dispatch_get_main_queue(), ^{
                ...
                if (self.appear == NO) {
                    self.appear = YES;
                    //UIImage *myImage = [UIImage imageNamed:@"detail_img_epic"];
                    ...
                    [playerLayer addSublayer:aLayer];
                    [self addIntroBellSound];
                }
                
                double delayInSecondss = 2.0; //0.1;
